...
module m28_isolator() {
    ...
    /** @brief A cover plate for the top of the battery. 
      * @todo Let the cover end flush with the m28_socket().
    ...
    /** @brief A hole to remove the battery isolator with one finger. For good printability, this 
      *   is a throughhole.
      * @todo Make the hole a 18 × 12 mm ellipse, as that is large enough for a finger.
      * @todo Add 45° chamfers to both ends of the hole. Use the BOSL cyl() shape for this. */
    module finger_hole() {
        h = m(part = "isolator", "cover h") + m(part = "socket", "middle section h");
        translate([m(part = "device", "base w") / 2, m(part = "device", "base d") - 14 , 0])
            scale([10/7.5, 1, 1])
                cylinder(r = 7.5, h = h + 2 * nothing);
    }
        ...
        union() {
            let($part = "device")
                translate([m("side overhang w"), m("front overhang d"), m(part = "isolator", "cover h") - nothing])
                    // @todo Shorten the ridge, as it interferes with the throughhole, and only the 
                    //   front part is necessary as a guide here since the isolator is only one-sided.
