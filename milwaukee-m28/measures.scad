...
function m(id, part = $part) = (
    ...
    part == "socket" && id == "corner radius w" ? 
        m(part = "socket", "middle section offset w"): // @todo rename to "h edges radius w"
    ...
    part == "socket" && id == "backwall d" ? 
        (m(part = "socket", "backwall d min") + m(part = "socket", "backwall d max")) / 2 :
    
    // Middle section, which is a separate part of the original Milwaukee battery socket.
    part == "socket" && id == "middle section h" ? 12.8 :
    part == "socket" && id == "middle section w" ? 
        m(part = "socket", "w") - 2 * m(part = "socket", "lock grooves offset w") 
        - 2 * m(part = "socket", "lock grooves w") :
    part == "socket" && id == "middle section offset w" ? 
        (m(part = "socket", "w") - m(part = "socket", "middle section w")) / 2 :
    ...
    part == "socket" && id == "ridge offset w" ? 
        m(part = "socket", "w") / 2 - m(part = "socket", "ridge w") / 2 :
    part == "socket" && id == "ridge offset d" ? 21.4 :
    part == "socket" && id == "ridge arcs d" ? 
        1.6 : // Unused. We simplify to a semi-circle arc via a radius the height edges, see below.
    part == "socket" && id == "ridge h edges r" ? 
        m(part = "socket", "ridge w") / 2 : // Maximum possible corner radius, yielding a semi-circle at the ends.
    ...
    part == "socket" && id == "lock grooves min d" ? 
        m("lock block offset d") + m("lock block d") + m(part = "battery", "play d") + 
        m(part = "battery", "snapper d") :
    part == "socket" && id == "lock grooves offset w" ? 3.3 :
    part == "socket" && id == "lock grooves offset h" ? 
        m(part = "socket", "side h") - m(part = "socket", "lock grooves h") :
    ...
    part == "socket" && id == "terminal holes outer w" ? 
        m(part = "socket", "terminal holes w") + 2 * m(part = "socket", "terminal holes chamfer t") :
    part == "socket" && id == "terminal holes outer h" ? 
        m(part = "socket", "terminal holes h") + 2 * m(part = "socket", "terminal holes chamfer t") :
    ...
    part == "device" && id == "front overhang d" ? 
        m(part = "device", "base d") - m(part = "device", "back overhang d") - m(part = "socket", "d") :
    part == "device" && id == "back overhang d" ? 1.5 :
    part == "device" && id == "side overhang w" ? 
        (m(part = "device", "base w") - m(part = "socket", "w")) / 2 :
